...
{
    PT_ENTRY *first = first_level_page_table[(vpage & PT_1) >> 10];
    if (first != NULL)
    {
      PT_ENTRY second = first[vpage & PT_2];
      if (second & PRES_BIT)
      {
        page_fault = FALSE;
        return (second & PAGE_FRAME);
      }
      else
      {
        page_fault = TRUE;
...
void pt_update_pagetable(VPAGE_NUMBER vpage, PAGEFRAME_NUMBER pframe)
{
  ...
  if (first == NULL)
  {
    first = (PT_ENTRY *) malloc(sizeof(PT_ENTRY) * PAGES_2);
  }

  first[vpage & PT_2] = (pframe | PRES_BIT);
...
{
  first_level_page_table[(vpage & PT_1) >> 10][vpage & PT_2] &= (~PRES_BIT);
}
